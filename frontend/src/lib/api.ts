...
function deriveRiskScore(pincode: string): number {
  const numericDigits = pincode.replace(/\D/g, '')
  const seed = numericDigits
    .split('')
    .reduce((sum, digit) => sum + Number(digit || 0), 0)

  const score = 1.8 + ((seed * 17) % 32) / 10
  return roundTo2(clamp(score, 1, 5))
}
...
function normalizeDeliveryType(deliveryType: string): 'hyperlocal' | 'same_day' | 'scheduled' {
  ...
  if (value === 'hyperlocal' || value === 'same_day' || value === 'scheduled') {
    return value
  }
  return 'same_day'
}
...
function estimatePremiumLocally(payload: PredictRequest): PremiumQuote {
  ...
  let base = 35
  if (weeklyEarnings >= 3000 && weeklyEarnings < 6000) {
    base = 60
  }
...
function buildDemoClaims(riderId: string): ClaimListItem[] {
  return [
    ...
    },
    {
      ...
      payout_amount: 960,
      held_amount: 640,
      servicenow_ticket_id: 'SN_MOCK_CLMYLW1',
    ...
    },
    {
      ...
      payout_amount: 0,
      held_amount: 1900,
      servicenow_ticket_id: 'SN_MOCK_CLMRED1',
...
function buildDemoDashboard(riderId: string): DashboardResponse {
  ...
  return {
    ...
    policy: {
      ...
      status: 'active',
      weekly_premium: 187,
      coverage_amount: 97240,
      no_claim_weeks: 3,
      next_payment_due: isoDaysAhead(4),
    },
    claims: claims.map((claim) => ({
      id: claim.id,
      trigger_id: claim.trigger_id ?? 'TRGDEMO',
...
function buildDemoClaimDetail(claimId: string): ClaimDetail {
  ...
  const found = claims.find((claim) => claim.id === claimId) ?? claims[0]
  return {
    claim_id: found.id,
    status: found.status,
    tier: found.tier,
    ...
    notifications: [
      {
        ...
        message: '₹187 due in 2 day(s)',
        time: isoDaysAgo(0),
      },
      {
        type: 'success',
        title: 'Payout Credited',
        message: '₹1,440 credited to your UPI (GREEN tier)',
        time: isoDaysAgo(2),
      },
      {
        type: 'alert',
        title: 'FLOOD Alert in Mumbai',
      ...
      },
      {
        ...
        message: '3 claim-free week(s) - 9% discount applied',
        time: isoDaysAgo(0),
      },
    ],
  }
...
function buildDemoAdminClaims(): ClaimListItem[] {
  const demoClaims = buildDemoClaims(DEMO_ACCOUNT.riderId)
  return [
    ...demoClaims,
    {
      id: 'CLM2201',
    ...
    },
    {
      ...
      payout_amount: 980,
      held_amount: 650,
      servicenow_ticket_id: 'SN_MOCK_CLM1021',
...
export async function registerRider(payload: RegisterRequest): Promise<RegisterResponse> {
  ...
  try {
    ...
    const riderId = payload.phone === DEMO_ACCOUNT.phone ? DEMO_ACCOUNT.riderId : `DEMO${Date.now().toString().slice(-4)}`
...
export async function getDashboard(riderId: string): Promise<DashboardResponse> {
  if (isDemoRider(riderId)) {
    return buildDemoDashboard(riderId)
  }
...
export async function getPolicy(riderId: string): Promise<PolicyResponse> {
  if (isDemoRider(riderId)) {
    return buildDemoPolicy(riderId)
  }
...
export async function getRiderProfile(riderId: string): Promise<RiderProfile> {
  if (isDemoRider(riderId)) {
    return buildDemoRiderProfile(riderId)
  }
...
export async function getNotifications(riderId: string): Promise<NotificationResponse> {
  if (isDemoRider(riderId)) {
    return buildDemoNotifications(riderId)
  }
...
export async function payNow(riderId: string): Promise<{ payment_link: string }> {
  if (isDemoRider(riderId)) {
    return { payment_link: 'https://rzp.io/demo-pay' }
  }
...
export async function subscribePremium(riderId: string): Promise<{ payment_link: string }> {
  if (isDemoRider(riderId)) {
    return { payment_link: 'https://rzp.io/demo-autodebit' }
  }
...
export async function getWeather(pincode: string): Promise<WeatherResponse> {
  try {
    ...
    if (isNetworkError(error)) {
      return buildDemoWeather(pincode)
    }
    throw error
  }
}
...
export async function getClaimsForRider(riderId: string): Promise<ClaimListItem[]> {
  ...
  return claims.filter((claim) => claim.rider_id === riderId)
}
...
export async function loginByPhone(phone: string): Promise<AdminRider | null> {
  ...
  return riders.find((rider) => rider.phone === phone) ?? null
}
